---
tags:
  - posts
id:
title: "Wired News: The Age of Cheap, Plastic Robots"
feature_image:
description: As a robot enthusiast and, dare I say, practitioner, there are three trends in consumer/entertainment robotics that I’m tracking and which…
date: 2006-08-15
full-date: 2006-08-15T21:44:45.000Z
slug: wired-news-the-age-of-cheap-plastic-robots
type: post
draft: false
status: done
---

As a robot enthusiast and, dare I say, practitioner, there are three trends in consumer/entertainment robotics that I’m tracking and which have opportunities for much deeper exploration.

* gladiator machines – [BattleBots](http://www.battlebots.com/). Where are the ballet robots? Machines with sensitivity? When will it become about the performance art and not the hobbyist exploitation? When will people be paying $100 a ticket to walk into a concert hall to watch machines perform?
* Maid robots – roomba, scooba, et al. With their reluctant acceptance of hacking [irobot](http://www.irobot.com/) has become an implicit supporter of robotic machine art. (see Make [Roomba serial hack](http://www.makezine.com/blog/archive/2006/02/how_to_make_a_roomba_serial_in.html) page). How else can we hack our modern appliances to make them do things they should not be doing? When will we be able to buy an appliance “blank” from the store and customize it to do exactly what we need in our own homes?
* Robot Toys – robot dogs ([aibo](https://en.wikipedia.org/wiki/AIBO) and the too-many-to-count variants) – what is the Lego of robotics (and [Mindstorms](http://mindstorms.lego.com/) doesn’t count… it’s too expensive and too constrained) – think “tinkertoy” non-proprietary components. When will robot pets breed?

(I disagree with Wired’s definition of robot, below. I would argue that a vast majority of robots don’t usually replicate complicated behaviors of human beings. They just perform rote mechanics, altering their behavior in response to inputs from their environment – more like bugs than simians).

[Wired News: The Age of Cheap, Plastic Robots](http://www.wired.com/news/columns/0,71602-0.html?tw=rss.index):  

> To be fair, these aren’t all robots in the scientific sense of “a machine designed to replicate the complicated behaviors of human beings,” but rather the marketing sense of, “You know, a robot! Like, a future thing, but cute. And it should have silver parts.”

_(via [weblogsky](http://feeds.feedburner.com/~r/Weblogsky/~3/13367715/001008.html))_


